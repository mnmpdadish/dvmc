...
int sumEigenEquation(int n_exc, double *e_ca, double *e_ac, double *u_ca, double *u_ac, double *u_cha, double *u_ahc, 
                                          double complex zh, double complex ze,  double *out_imG_ca, double *out_imG_ac)

  // to be able to use ctrl-c
  // signal(SIGINT, interrupt_handler); 
  //
  int nn
  double complex g_ca=0.0;
  double complex g_ac=0.0;
  for(nn=0;nn<n_exc;nn++){
     g_ca += u_ca[nn]*us_ca[nn] / (zh + e_ca[nn]);
     g_ac += u_ac[nn]*us_ac[nn] / (ze - e_ac[nn]);
  }    
  out_imG_ca[0]=imag(g_ca);
  out_imG_ca[0]=imag(g_ca);
