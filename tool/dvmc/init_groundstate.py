...
for k in range(Nsite): # BZ
    ...
    Disp[k] = -2.0 * T1 * ( cos(kwave[0]) + cos(kwave[1]) ) \
        ...
        -4.0 * T4 * ( cos(2.0*kwave[0])*cos(kwave[1]) + cos(kwave[0])*cos(2.0*kwave[1]) ) \
...
#print 2+Ngut+Njastrow+6*Ndh2+10*Ndh4
#print Ngut,Njastrow,Ndh2,Ndh4,NOrbital

# print out
fname='zqp_opt.dat'
f = open(fname,'w')
for idx in range(2+Ngut+Njastrow+6*Ndh2+10*Ndh4):
    #print idx, '{0:25.18e} {1:25.18e} 0.0' .format(0.0,0.0)
    f.write('{0:25.18e} {1:25.18e} 0.0 '.format(0.0,0.0))
for idx in range(NOrbital):
    #print 42+idx, '{0:25.18e} {1:25.18e} 0.0 '.format(fRe[i+Nsite*j],0.0)
    i = orbitalSiteI[idx]
    j = orbitalSiteJ[idx]
    if APFlag==1:
        isub = subIndex(i)
