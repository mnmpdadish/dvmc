...
NOrbital_parallel = Nsub*(Nsite-1) # the i=j terms are excluded 
Norbital_antiparallel = Nsub*Nsite
NOrbital = 2*NOrbital_parallel + Norbital_antiparallel

idx = 0
orbital_parallel=[[-1 for dj in range(Nsite)] for isub in range(Nsub)]
for isub in range(Nsub):
    for dj in range(Nsite):  # dj=0 is excluded
        if dj>0:
            orbital_parallel[isub][dj] = idx
            idx += 1

idx = 0
orbital_antiparallel=[[-1 for dj in range(Nsite)] for isub in range(Nsub)]
for isub in range(Nsub):
    for dj in range(Nsite):
        orbital_antiparallel[isub][dj] = idx
        idx += 1


# print out
f = open('inorbital_general.dat','w')
f.write(separator+
        "NOrbitalGeneralIdx\t"+ str(NOrbital) +"\n"+
        separator+
        "i_j_OrbitalIdx\n"+
        separator)
# orbital_parallel
for s in range(2):
    for isub in range(Nsub):
        ...
        rx=isub%Sx
        ry=(isub-rx)/Sx
        i=positionToIndex((rx,ry))
        for dj in range(Nsite):
            if dj>0:
                dr=indexToPosition(dj)
                j=neighborIndex(i,dr)

                # rotate spin to X-axis
                if s==0: # s=t=0
                   #fijst = fRe[i+Nsite*j]*sgn *0.5
                    fijst = 0.0
                elif s==1: # s=t=1
                   #fijst = -fRe[i+Nsite*j]*sgn *0.5
                    fijst = 0.0

                offset = s*NOrbital_parallel
                idx = offset+orbital_parallel[isub][dj]
                f.write('{0}\t{1:25.18e} {2:25.18e}\n'.format(idx,fijst,0.0))

# orbital_antiparallel
for isub in range(Nsub):
    #isub=rx+ry*Sx
    rx=isub%Sx
    ry=(isub-rx)/Sx
    ...
    for dj in range(Nsite):
        ...
        offset = 2*NOrbital_parallel
        idx=offset+orbital_antiparallel[isub][dj]
        f.write('{0}\t{1:25.18e} {2:25.18e}\n'.format(idx,fijst,0.0))
